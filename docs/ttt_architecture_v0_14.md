...
domain-neutral Python middleware library for conversational turn processing. It emerged from design work on **Adjacency** (a structured elicitation research instrument) and the need for a reusable infrastructure layer that could serve that project and others.
...
- `subscriptions: list[SubscriptionLike]` (event patterns; may include globs, e.g., `Events.*`)
...
Subscriptions are matched against `HubEvent.event_type` using simple glob semantics.

**Canonical contract (routing vs filtering):**
- **Route** by `event_type` only.
- **Filter** optionally by attributes (e.g., `turn_id`, `purpose_id`).
- Turn scoping uses `turn_id` only.
- Subscription entries MAY be either a string pattern or an object with `event_type` + optional filters.
...
Turn scoping is expressed via **attribute filtering** on `HubEvent.turn_id` (and optionally other well-known attributes), not by encoding a turn identifier into the `event_type` pattern. Do **not** rely on raw text matching.
...
> Note: the strings inside `add_subs` / `delete_subs` are subscription patterns (e.g., `Events.*`), not turn identifiers. If you need turn scoping, the hub should filter/route based on the event’s `turn_id` or a separate explicit filter mechanism.
...
(See **Canonical contract (routing vs filtering)** in §4.3.2.)
...
Notes:
- `conversation_id`/`turn_index` are nullable to support non-turn-scoped events (e.g., `purpose_registration`, `subscriptions_changed`).
